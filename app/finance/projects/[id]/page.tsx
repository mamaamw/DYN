...
import { ArrowLeft, Wallet, TrendingDown, Calendar, DollarSign, Plus, Edit2, Trash2 } from 'lucide-react';
...
export default function ProjectDetailPage() {
  ...
  const fetchProject = async () => {
    ...
    try {
      const res = await fetch(`/api/finance/projects/${params.id}`);
      if (res.ok) {
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    if (!expenseForm.name || !expenseForm.amount || !expenseForm.walletId) {
  ...
  const getTotalWalletsAmount = () => {
    if (!project?.wallets) return { amount: 0, currency: 'BTC', mixed: false };
    
    const currencies = [...new Set(project.wallets.map(w => w.currency))];
    const mixed = currencies.length > 1;
    
    if (mixed) {
      return { amount: 0, currency: 'Mixte', mixed: true };
    }
    
    const total = project.wallets.reduce((sum, w) => sum + w.amount, 0);
    return { amount: total, currency: currencies[0] || 'BTC', mixed: false };
  };

  const getTotalExpenses = () => {
    if (!project?.expenses) return { amount: 0, currency: 'BTC', mixed: false };
    ...
    const total = project.expenses.reduce((sum, e) => sum + e.amount, 0);
    return { amount: total, currency: currencies[0] || 'BTC', mixed: false };
  };
  ...
  const walletsTotal = getTotalWalletsAmount();
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <Button
            variant="secondary"
            onClick={() => router.push('/finance?tab=projects')}
            className="mb-4"
          >
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">
                  {walletsTotal.mixed ? (
                    ...
                    `${getCurrencySymbol(walletsTotal.currency)} ${formatAmount(walletsTotal.amount, walletsTotal.currency)}`
                  )}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">
                  {expensesTotal.mixed ? (
                    ...
                    `${getCurrencySymbol(expensesTotal.currency)} ${formatAmount(expensesTotal.amount, expensesTotal.currency)}`
                  )}
                </p>
              </div>
        ...
        <div className="flex gap-3 mb-6">
          ...
          <Button
            onClick={() => router.push(`/finance?tab=projects&edit=${project.id}`)}
            variant="secondary"
          >
        ...
        {(project.startDate || project.endDate) && (
          <Card className="p-4 mb-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Dates</h2>
        ...
        <Card className="p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
            Portefeuilles ({project.wallets.length})
          </h2>
          
          {project.wallets.length === 0 ? (
            <p className="text-gray-600 dark:text-gray-400 text-center py-8">
              Aucun portefeuille associé à ce projet
            </p>
          ) : (
        ...
        <Card className="p-6">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
            Dépenses ({project.expenses.length})
          </h2>
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                  {project.expenses.map((expense) => (
                    <tr key={expense.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                        <button
                          onClick={() => router.push(`/finance/wallets/${expense.wallet.id}`)}
                    ...
                    >
                      ...
                      {wallets.map((wallet) => (
                        <option key={wallet.id} value={wallet.id}>
                          {wallet.name} ({getCurrencySymbol(wallet.currency)} {formatAmount(wallet.amount, wallet.currency)})
                        </option>
                      ))}
                    </select>
                  </div>
