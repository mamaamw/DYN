...
export default function EditClientPage() {
  ...
  const fetchClient = async () => {
    try {
      ...
      const data: NewClient = await response.json();
      ...
      setFormData({
        ...
        externalHelp: data.externalHelp || false,
        generalReference: search?.generalReference || '',
        detailedReference: search?.detailedReference || '',
        searchStartDate: search?.startDate ? search.startDate.split('T')[0] : '',
        searchEndDate: search?.endDate ? search.endDate.split('T')[0] : '',
      });
      
      setContacts(data.contactIdentifiers.map(c => ({
        id: c.id,
      ...
      const initialState = JSON.stringify({
        ...
        contacts: data.contactIdentifiers.map(c => ({
          accountNumber: c.accountNumber,
          accountType: c.accountType,
          info: c.info || '',
        }))
  ...
  const validateNickname = async (nickname: string) => {
    ...
    try {
      const response = await fetch(`/api/newclients/${id}/validate`, {
        method: 'POST',
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setHasChanges(currentState !== initialData);
    }
  }, [formData, contacts, initialData]);
  ...
  const validateContactsRealTime = async (contactsToValidate: ContactIdentifier[]) => {
    ...
    // Vérifier les doublons avec la base de données (sauf pour ce client)
    try {
      ...
      if (response.ok) {
        ...
        if (data.warnings && data.warnings.length > 0) {
          ...
          const warningsMessage = `Attention - Ces numéros sont utilisés avec d'autres types de comptes:\n${warningsList}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Réinitialiser hasChanges après sauvegarde réussie
      setHasChanges(false);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#121a2d]">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={handleBack}
              className="p-2 hover:bg-gray-100 dark:hover:bg-[#1b2436] rounded-lg"
            >
              <ArrowLeft size={20} className="text-gray-600 dark:text-gray-400" />
            </button>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              Éditer le client
              {hasChanges && <span className="text-orange-500 text-sm ml-2">●</span>}
            </h1>
          </div>
        </div>
      </header>
                ...
                {loadingCategories ? (
                  ...
                  <>
                    ...
                    <p className="mt-2 text-sm text-blue-600 dark:text-blue-400">
                      ℹ️ Catégories disponibles: {userCategories.map(c => `${c.icon} ${c.label}`).join(', ')}
                    </p>
                  </>
                ) : (
                  <div className="w-full px-3 py-2 border border-gray-300 dark:border-[#1b2436] rounded-lg bg-gray-50 dark:bg-[#121a2d]">
                    <span className="text-gray-500 dark:text-gray-400">Aucune catégorie disponible. Contactez un administrateur.</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {showConfirmDialog && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white dark:bg-[#0f172a] rounded-lg shadow-xl max-w-md w-full mx-4 border dark:border-[#1b2436]">
            <div className="p-6">
